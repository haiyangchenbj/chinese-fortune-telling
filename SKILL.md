...
出生地不在 cantian 内置城市表内（如民权、兰考等县级地名）时**必须传十进制经度**，不能用邻近地级市代替。
...
输出前逐条过：真太阳时是否校正 → 夏令时是否扣回 → 时辰是否贴边界（贴了就双盘并陈）→ 生日当天是否恰逢节气交接（是则月柱按交接时刻判定）→ 大运换运年份是否 10 年等差且与起运日期自洽（3 天折 1 岁；逆行取上一节、顺行取下一节）→ 十神占比与四柱是否自洽 → **强弱分与用神是否只取自 `bazi-analysis.js`（若同时跑了 `ziwei.js`，其「八字」「月令」「格局」「用神」四块是否已屏蔽）** → 有无绝对断言。

### Step 8 [Deterministic] 落盘
...
按问题类型收口，长度受控，不堆神煞、不重复免责声明。

**八字／紫微完整命盘**（800–1500 字）：
1. 命盘速览（四柱＋日主＋格局＋调候用神，一句话总断）
2. 核心特征 3–5 条
3. 分维度解读（按用户所问，未问的不铺开）
4. 大运／流年与应期
5. 行动建议（宜／缓／避）
6. 流派声明与免责（一段，不重复）

**流年问答** 300–600 字｜**六爻／梅花单次占断** 300–500 字｜**择吉** 以表格给候选日＋评分＋宜忌，200 字内说明。
