 ...
 * 六爻预测脚本
 * 模拟三枚铜钱摇六次成卦
 * 输入：6个数字（每位一爻）：0=阳（少阳，不动），1=阴（少阴，不动），
 *       2=老阳（阳动，变爻），3=老阴（阴动，变爻）
 */
...
/**
 * 抛铜钱模拟（口径与下方实现及 parseCoins 严格一致）
 * 3个铜钱，正面=阳面(记1)，背面=阴面(记0)；heads = 阳面数
 * 3正（全阳）= 老阳 → 阳动（变爻）
 * 2正1背（1阳面）= 少阴 → 阴（不变）
 * 1正2背（2阳面）= 少阳 → 阳（不变）
 * 3背（全阴）= 老阴 → 阴动（变爻）
...
/**
 * 从输入解析卦象
 * 输入格式：6个数字组成的字符串（每位一爻，与文件头说明一致）
 * 0=阳（少阳，不动），1=阴（少阴，不动），2=老阳（阳动），3=老阴（阴动）
 */
